...
var generateGetPosText = function generateGetPosText () {
    return [
        'vec2 getPosText(const in vec3 position) {',
        '  float sposition = position.x + position.y * iRealSize.x + position.z * iRealSize.x * iRealSize.y;',
        '  return vec2(',
...
var generateGetPixelGlsl = function generateGetPixelGlsl (outOfBoundValue) {
    ...
    if (outOfBoundValue === 'wrap') {
        return [
            ...
            '  vec3 position = mod(currentPos + add, iRealSize);',
...
var generateGetNeighbourhood = function (neighbourhood) {
    var glsl = [
        ...
        '  float sposition = currentPos.x + currentPos.y * iTextureSize.x;',
        '  vec3 pixelPos = vec3(',
        '    mod(sposition, iRealSize.x),',
        '    mod(floor(sposition / iRealSize.x), iRealSize.y),',
        '    floor(sposition / (iRealSize.x * iRealSize.y))',
        '  );',
        '  int sum = 0;',
        ''
    ];
...
var generateUniformsAndConstants = function generateUniformsAndConstants (dimensions, textureWidth, textureHeight) {
    return [
        ...
        'const vec3 iStride = vec3(0, ' + dimensions[0] + ', ' + (dimensions[0] * dimensions[1]) + ');',
