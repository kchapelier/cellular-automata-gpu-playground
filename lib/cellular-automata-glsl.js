...
var generateGetPixelGlsl = function generateGetPixelGlsl (outOfBoundValue) {
    ...
    if (outOfBoundValue === 'wrap') {
        return [
            ...
            '  return int(texture2D(iBackbuffer, pixelPos).x);',
            '}'
        ].join('\n');
    } else {
        return [
...
var generateProcessGlslLife = function generateProcessGlslLife (survival, birth) {
    ...
    var glsl = [
        'float process(const in float currentValue, const in int sum) {',
        '  if (currentValue < 0.9 && (' + birthString.join(' || ') + ')) {',
        '    return 1.;',
        '  } else if (currentValue > 0.9 && (' + survivalString.join(' || ') + ')) {',
        '    return 1.;',
        '  }',
        '  return 0.;',
        '}'
...
var generateProcessGlslLuky = function generateProcessGlslLuky (lowSurvival, highSurvival, lowBirth, highBirth) {
    var glsl = [
        'float process(const in float currentValue, const in int sum) {',
        '  if (currentValue < 0.9 && sum >= ' + lowBirth + ' && sum <= ' + highBirth + ') {',
        '    return 1.;',
        '  } else if (currentValue > 0.9 && sum >= ' + lowSurvival + ' && sum <= ' + highSurvival + ') {',
        '    return 1.;',
        '  }',
        '  return 0.;',
        '}'
...
module.exports = function generateShaders(rule, neighbourhood, dimensions, outOfBoundValue) {
    ...
    var fragmentGlsl = [
        ...
        '',
        'void main() {',
        '  gl_FragColor = texture2D(iBackbuffer, gl_FragCoord.xy / iResolution);',
        '  gl_FragColor = vec4(process(gl_FragColor.r, getNeighbourhood(gl_FragCoord.xy)));',
        '}',
        ''
    ];
