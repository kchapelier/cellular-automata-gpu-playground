...
var generateGetPixelGlsl = function generateGetPixelGlsl (outOfBoundValue) {
    ...
    if (outOfBoundValue === 'wrap') {
        return [
            'int getPixel(const in vec2 resolution, const in vec2 currentPos, const in vec2 add) {',
            '  vec2 pixelPos = fract((currentPos + add) / resolution);',
...
var generateProcessGlsl = function generateProcessGlsl (rule) {
    ...
    */
    ...
    if (rule.ruleFormat === 'life' || rule.ruleFormat === 'extended') {
        return generateProcessGlslLife(rule.survival, rule.birth);
    } else if (rule.ruleFormat === 'generations') {
...
module.exports = function generateShaders(rule, neighbourhood, dimensions, outOfBoundValue) {
